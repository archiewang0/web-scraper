...
import { PlusCircle, ArrowRight, Trash2, Plus } from 'lucide-react'
...
export default function SettingPage() {
    ...
    const handleFinish = async () => {
        ...
        try {
            ...
            // 存儲結果到 sessionStorage
            sessionStorage.setItem('scrapeResults', JSON.stringify(results));
            router.push('/results');
        } catch (error) {
            console.error('Scraping failed:', error);
            alert('Failed to scrape the websites. Please check your configurations.');
        } finally {
            setIsLoading(false);
        }
    };
                ...
                <CardContent>
                    {configurations.length === 0 ? (
                        ...
                        <div className="space-y-4">
                            {configurations.map((config) => (
                                <Card key={config.id} className="p-4 border border-gray-200 hover:border-gray-300 transition-colors">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <h3 className="text-lg font-medium">{config.name || '未命名設定'}</h3>
                                            <p className="text-sm text-gray-500 truncate max-w-md">{config.url}</p>
                                            <p className="text-xs text-gray-400 mt-1">
                                                已設定 {config.fields.filter(f => f.selector).length} 個選擇器
                                            </p>
                                        </div>
